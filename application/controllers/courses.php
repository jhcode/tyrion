...
class Courses extends MY_Controller
{
	...
	public function add($course_id = false, $type = false)
	{
		...
		if ($this->form_validation->run()) {
			...
			if($type === 'quiz'):
				$question = [];
				$question['question'] = $data['details'];
				$question['options'] = $data['option'];
				$question['answers'] = $data['answer'];
				$questions = ['instruction'=>$data['instruction'],'questions' => $question];
				$questions['duration'] = $data['time_hrs'].':'.$data['time_mins'];
				$details = json_encode($questions);
				$post = [
				'type' => $type,
				'course_id' => $course_id,
				'title' => $data['title'],
