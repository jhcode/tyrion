...
class MY_Model extends CI_Model
{
    ...
    protected $before_create = array('_prefixor');
    protected $after_create = array('_deprefixor');
    protected $before_update = array('_prefixor');
    protected $after_update = array('_deprefixor');
    protected $before_get = array('_prefixor');
    protected $after_get = array('_deprefixor');
    protected $before_delete = array('_prefixor');
    protected $after_delete = array('_deprefixor');
    protected $before_count = array('_prefixor');
    protected $after_count = array('_deprefixor');
    ...
    /**
    *
    * Gets the last entry from a database
    * @param column_name
    */
    public function get_last_entry($column_name = false)
    {
        if(!$column_name)return false;

        $this->db->select($column_name);
        $this->order_by($column_name,'desc');
        $this->limit(1);
        $result = $this->get_all();
    ...
    function dropdown()
    {
        ...
        foreach ($result as $row)
        {
            ...
            {
                $dropdown .= '<option value="'.$row->{$key}.'">'.ucfirst($row->{$value}.' '.$row->lastname).'</option>';
            }else
            {
                $dropdown .= '<option value="'.$row->{$key}.'">'.ucfirst($row->{$value}).'</option>';
            }
        }
    ...
    public function count_all()
    {
        ...
        $result = $this->db->count_all($this->_table);
    ...
    {             
        $prefix = $this->session->userdata('company_id').'_';   
        return $this->db->select('AUTO_INCREMENT')
            ->from('information_schema.TABLES')
            ->where('TABLE_NAME',$prefix.$this->_table)
            ->where('TABLE_SCHEMA', $this->db->database)->get()
            ->row()->AUTO_INCREMENT;               
    }
     ...
     */
    protected function _run_before_callbacks($type, $params = array())
    {
        $name = 'before_' . $type;
     ...
     * variable and returning it
     */
    protected function _run_after_callbacks($type, $params = array())
    {
        $name = 'after_' . $type;
        $data = (isset($params[0])) ? $params[0] : FALSE;
    ...
    public function transaction_error() {
        if ($this->db->trans_status() === FALSE)
        {
            ...
            return "An Error Occured while trying to query the database.";
        }
    }
    ...
    public function _prefixor($data = true) { 

        return $data;
    }
    
    public function _deprefixor($data = true) {
    ...
    public function verify_user($params = false)
    {   
        if(!(bool)$params)
        {
            return false;
        ...
        $user = $this->user->get_by(array('email'=>$params['email']));

        if(!$user):
            return false;
        endif;

        $is_valid_password = $this->verify_crypt($params['password'],$user->password);
    ...
    */
    public function encrypt($input = false,$rounds = 7)
    {
    ...
    /**
    *Verify crypt
    *Blowfish verifiction mechanism
    *Algorithm  
    *@param arg - string to be verified(string)
    *@param hash - stored has of string(string)
    *@return (bool)
    */
    public function verify_crypt($arg,$hashed_pass)
    {
        if(!$arg)return false;

        if (crypt($arg, $hashed_pass) == $hashed_pass)return true;
