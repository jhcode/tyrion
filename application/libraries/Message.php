...
class CI_Message {
	...
	private $message_folder	= 'messages/';
	...
	function set($newdata = array(), $newvalue = NULL)
	{
		...
		{			
			foreach ($newdata as $group => $value):
				...
				//don't allow duplicate messages to be logged
				$msgs = $this->CI->session->userdata('_messages');
			
				if(!(bool)$msgs[$group] || !in_array($value,$msgs[$group])){
